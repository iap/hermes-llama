...
### Documentation
- Added a **terminology disambiguation** section to `README.md` and the
  `llama-cpp-local-models` skill, clarifying that **llama.cpp** (the engine),
  **"Llama CPP"** (the provider label), **Ollama** (a separate serving
  toolchain — not integrated), **LFM/LiquidAI** (a model weight family), and
  **GGUF** (the file format) are distinct concepts. Prevents an agent or a
  new reader conflating them.

## [0.2.2] - 2026-09-09

### Added
- **Archive digest persistence + tamper evidence** — the install meta now records `archive_sha256` per tag (streaming hash; stdlib-only). Reinstalling the same tag with a changed archive appends a **TAMPER WARNING** to the result detail. `check()` returns the recorded digest in its dict output. Three regression tests cover persistence, the warning on digest change, and the no-false-positive guard.
